...
def get_driver_path(driver_name):
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        ...
        # __file__ é o caminho para este script (ex: price-watcher/scrapers/kabum.py)
...
# --- Função de scraping para Kabum ---
# Recebe 'time_str' para adicionar ao dicionário de produto
def kabum(produto, page_number, time_str): # Renomeado 'number' para 'page_number' para clareza
    ...
    # ATENÇÃO: URL ajustada para busca genérica na Kabum
    url = f"https://www.kabum.com.br/busca/{produto}?page_number={page_number}&page_size=20&facet_filters=&sort=most_searched"
    driver.get(url)
    ...
    # Seus seletores 'nameCard' e 'priceCard'
    # ATENÇÃO: Verifique se esses seletores ainda funcionam na Kabum.
    # Eles podem ser dinâmicos ou mudar. Se não coletar dados, precisaremos inspecionar novamente.
    nomes_elems = soup.find_all(class_=re.compile(r"\bnameCard\b"))
    preco_elems = soup.find_all(class_=re.compile(r"\bpriceCard\b"))
